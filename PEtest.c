...
int main(int argc, char *argv[])
{
	...
	////////////////////////////////////////////////////////////////////////
	printf("Characterristics : %#lx\n", FileHeader.Characteristics);
	scanf_s("%d", &Character_modification);
	printf("%d\n", Character_modification);
	fclose(p);
	fopen_s(&p, argv[1], "wb");
	fseek(p, PointerToPeHeader+sizeof(WORD)*3 +sizeof(DWORD)*3, SEEK_CUR);
	fwrite(&Character_modification,sizeof(Character_modification),1, p);
	fclose(p);
	fopen_s(&p, argv[1], "rb");
	fseek(p, PointerToPeHeader, SEEK_SET);
	//fseek(p, sizeof(WORD) * 3 + sizeof(DWORD) * 3+ sizeof(Character_modification), SEEK_CUR);
